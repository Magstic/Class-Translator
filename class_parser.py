...
class ClassParser:
    ...
    def _parse(self):
        ...
        # The constant pool is 1-indexed. We use a while loop to handle multi-slot constants.
        i = 1
        while i < cp_count:
            ...
            if tag == CONSTANT_Utf8:
                ...
                # Check if we are past the constant pool due to a final Long/Double
                if i == cp_count - 1 and len(self.constant_pool) == cp_count - 2:
                    # This can happen if the last entry was a Long/Double, loop should have ended
                    break
                raise ValueError(
            ...
            if is_long_or_double:
                self.constant_pool.append(None)  # Add a placeholder for the second slot
                i += 2  # Skip the next entry
            else:
                i += 1

        # Store the rest of the file data that comes after the constant pool
    ...
    def update_utf8_string(self, index, new_string):
        ...
        # Create the new raw entry: tag (1 byte), length (2 bytes), data
    ...
    def save(self, new_filepath):
        ...
        # Write the rest of the file data
